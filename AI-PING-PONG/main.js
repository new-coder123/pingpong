...
function preload() {
 x = loadSound("ball_touch_paddel.wav")
}
...
function setup() {
    ...
    video.size(800,450)
    posenet = ml5.poseNet(video,modleloadded)
...
function modleloadded(){
console.log("working");
}

function gotposses(res){
    if(res.length>0){
...
if(pcscore ==4){
    ...
    text("press the reload button!",width/2,height/2+30)
    noLoop();
    pcscore = 0;
}
